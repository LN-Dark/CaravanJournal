...
import com.luanegra.caravanjournal.adapters.PersonaNonGrataLocationsAdapter
...
class PersonaNonGrataFragment : Fragment() {
    ...
    private var locationAdapter: PersonaNonGrataLocationsAdapter? = null
    private var recycler_locations: RecyclerView? = null
    private var tiet_search_locations_personanongrata: TextInputEditText? = null
    ...
    override fun onCreateView(
        ...
        recycler_locations = root!!.findViewById(R.id.recycler_locations_personanongrata)
        recycler_locations!!.setHasFixedSize(true)
        val linearLayoutManager = LinearLayoutManager(context)
        linearLayoutManager.stackFromEnd = false
        recycler_locations!!.layoutManager = linearLayoutManager
        tiet_search_locations_personanongrata = root!!.findViewById(R.id.tiet_search_locations_personanongrata)
        tiet_search_locations_personanongrata!!.addTextChangedListener(object : TextWatcher {
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (snapshot.exists()) {
                    for (user in snapshot.children) {
                        ...
                        if (context != null) {
                            locationAdapter = PersonaNonGrataLocationsAdapter(context!!, mPersonaNonGrata!!)
    ...
    private fun createNewLocation(){
        ...
        mDialogView.findViewById<Button>(R.id.save_dialog).setOnClickListener {
            if (!tiet_add_locations_dialog.text.isNullOrEmpty()) {
                ...
                if (!alreadyExists){
                    var refLocations: DatabaseReference? = null
                    refLocations = FirebaseDatabase.getInstance().reference.child("users").child(
                        FirebaseAuth.getInstance().currentUser.uid).child("Locations")
    ...
    private fun retrieveSearchLocations(searchname: String) {
        if(searchname != ""){
            val queryUsers = FirebaseDatabase.getInstance().reference.child("PersonaNonGrata")
                ...
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    if (snapshot.exists()) {
                        for (location in snapshot.children) {
                            val newLocation: PersonanonGrata_Location? = location.getValue(PersonanonGrata_Location::class.java)
